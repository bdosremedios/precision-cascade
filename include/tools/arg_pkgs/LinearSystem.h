...
protected:
    ...
    GenericLinearSystem<M> * const lin_sys_ptr;

public:

    TypedLinearSystem_Base(GenericLinearSystem<M> * const arg_lin_sys_ptr):
        lin_sys_ptr(arg_lin_sys_ptr)
    {}

    // Wrapper behavior, wrapping GenericLinearSystem<M> methods
    const M<double> &get_A() const { return lin_sys_ptr->get_A(); }
    const Vector<double> &get_b() const { return lin_sys_ptr->get_b(); }
    const int get_m() const { return lin_sys_ptr->get_m(); }
    const int get_n() const { return lin_sys_ptr->get_n(); }
    const int get_nnz() const { return lin_sys_ptr->get_nnz(); }
    const cuHandleBundle get_cu_handles() const { return lin_sys_ptr->get_cu_handles(); }
...
public:
    ...
    TypedLinearSystem(GenericLinearSystem<M> * const arg_lin_sys_ptr):
        TypedLinearSystem_Base<M, T>(arg_lin_sys_ptr),
        A_typed(this->lin_sys_ptr->get_A().template cast<T>()),
        b_typed(this->lin_sys_ptr->get_b().template cast<T>())
    {}
...
public:
    ...
    TypedLinearSystem(GenericLinearSystem<M> * const arg_lin_sys_ptr):
        TypedLinearSystem_Base<M, double>(arg_lin_sys_ptr)
    {}

    // *** Implemented virtual methods ***
    const M<double> &get_A_typed() const override { return this->lin_sys_ptr->get_A(); }
    const Vector<double> &get_b_typed() const override { return this->lin_sys_ptr->get_b(); }
    void set_b(const Vector<double> &arg_b) override { this->lin_sys_ptr->set_b(arg_b); }
