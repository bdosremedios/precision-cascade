...
public:
        ...
        TypedIterativeSolve<NoFillMatrixSparse, T>::TypedIterativeSolve(arg_typed_lin_sys, arg_pkg)
    {
        ...
        int *new_col_offsets = static_cast<int *>(malloc((typed_lin_sys.get_m()+1)*sizeof(int)));
        int *new_row_indices = static_cast<int *>(malloc(typed_lin_sys.get_m()*sizeof(int)));
        T *new_vals = static_cast<T *>(malloc(typed_lin_sys.get_m()*sizeof(T)));

        for (int i=0; i<typed_lin_sys.get_m()+1; ++i) { new_col_offsets[i] = i; }
        for (int i=0; i<typed_lin_sys.get_m(); ++i) { new_row_indices[i] = i; }
        for (int i=0; i<typed_lin_sys.get_m(); ++i) {
            new_vals[i] = 1./typed_lin_sys.get_A().get_elem(i, i).get_scalar();
        }

        D_inv = NoFillMatrixSparse<T>(
            arg_typed_lin_sys.get_cu_handles(),
            new_col_offsets, new_row_indices, new_vals,
            typed_lin_sys.get_m(), typed_lin_sys.get_n(), typed_lin_sys.get_m()
        );
