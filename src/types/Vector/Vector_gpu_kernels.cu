...
__global__ void vector_dbl_kernels::cast_to_float(double *vec_src, float *vec_dest, int m) {
    int tid = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (tid < m) {
...
__global__ void vector_sgl_kernels::cast_to_double(float *vec_src, double *vec_dest, int m) {
    int tid = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (tid < m) {
...
// *** Vector half kernel implementations ***

__global__ void vector_hlf_kernels::cast_to_float(__half *scalar_src, float *scalar_dest, int m) {
    int tid = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (tid < m) {
...
__global__ void vector_hlf_kernels::cast_to_double(__half *scalar_src, double *scalar_dest, int m) {
    int tid = (blockIdx.x * blockDim.x) + threadIdx.x;
    if (tid < m) {
