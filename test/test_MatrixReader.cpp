...
class MatrixReadGeneralTest: public testing::Test {
...
class MatrixReadDoubleTest: public testing::Test {

    protected:
        string matrix_dir = "/home/bdosremedios/dev/gmres/test/read_matrices/";
        double machine_eps = pow(2, -52);
        double tol = machine_eps;
...
TEST_F(MatrixReadDoubleTest, ReadSquareMatrix) {
    ...
    for (int i=0; i<3; ++i) {
        for (int j=0; j<3; ++j) {
            EXPECT_NEAR(test1(i, j), target1(i, j), tol);
        }
    }
    ...
    for (int i=0; i<5; ++i) {
        for (int j=0; j<5; ++j) {
            EXPECT_NEAR(test2(i, j), target2(i, j), tol);
        }
    }
...
TEST_F(MatrixReadDoubleTest, ReadWideTallMatrix) {
    ...
    for (int i=0; i<2; ++i) {
        for (int j=0; j<5; ++j) {
            EXPECT_NEAR(test_wide(i, j), target_wide(i, j), tol);
        }
    }
    ...
    for (int i=0; i<4; ++i) {
        for (int j=0; j<2; ++j) {
            EXPECT_NEAR(test_tall(i, j), target_tall(i, j), tol);
        }
    }
...
class MatrixReadSingleTest: public testing::Test {

    protected:
        string matrix_dir = "/home/bdosremedios/dev/gmres/test/read_matrices/";
        double machine_eps = pow(2, -23);
        double tol = machine_eps;
...
TEST_F(MatrixReadSingleTest, ReadPreciseMatrix) {
    ...
    string precise_file = matrix_dir + "single_precise.csv";
...
TEST_F(MatrixReadSingleTest, ReadDifferentThanPreciseMatrix) {
    ...
    float eps = static_cast<float>(1.5*tol);
    MatrixXf miss_precise_up {{static_cast<float>(1.12345672)+eps, static_cast<float>(1.12345674)+eps},
                              {static_cast<float>(1.12345676)+eps, static_cast<float>(1.12345678)+eps}};
    MatrixXf miss_precise_down {{static_cast<float>(1.12345672)-eps, static_cast<float>(1.12345674)-eps},
                                {static_cast<float>(1.12345676)-eps, static_cast<float>(1.12345678)-eps}};
    string precise_file = matrix_dir + "single_precise.csv";
    MatrixXf test_precise(read_matrix_csv<float>(precise_file));
...
class MatrixReadHalfTest: public testing::Test {

    protected:
        string matrix_dir = "/home/bdosremedios/dev/gmres/test/read_matrices/";
        double machine_eps = pow(2, -10);
        double tol = machine_eps;
...
TEST_F(MatrixReadHalfTest, ReadPreciseMatrix) {
    ...
    string precise_file = matrix_dir + "half_precise.csv";
...
TEST_F(MatrixReadHalfTest, ReadDifferentThanPreciseMatrix) {
    ...
    half eps = static_cast<half>(1.5*tol);
