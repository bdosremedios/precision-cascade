...
public:
    ...
    void TestManualSort() {
        ...
        T arr_1[n] = {
            static_cast<T>(2.), static_cast<T>(1.), static_cast<T>(9.),
            static_cast<T>(4.), static_cast<T>(7.), static_cast<T>(8.),
            static_cast<T>(5.), static_cast<T>(3.), static_cast<T>(6.)
        };
        W arr_2[n] = {
            static_cast<W>(1.), static_cast<W>(2.), static_cast<W>(3.),
        ...
        W arr_2_soln[n] = {
            static_cast<W>(2.), static_cast<W>(1.), static_cast<W>(8.),
    ...
    void TestRandomSort() {
        ...
        std::vector<T> arr_1;
        for (int k=0; k<n; ++k) {
            arr_1.push_back(static_cast<T>(k));
        }
        std::vector<W> arr_2;
        for (int k=0; k<n; ++k) {
            arr_2.push_back(static_cast<W>(k));
        }

        std::shuffle(
            arr_1.begin(), arr_1.end(), std::default_random_engine(rand())
        );

        std::vector<T> arr_1_soln;
        for (int k=0; k<n; ++k) {
            arr_1_soln.push_back(static_cast<T>(k));
        }
        std::vector<W> arr_2_soln;
        arr_2_soln.resize(n);
        for (int k=0; k<n; ++k) {
            arr_2_soln[static_cast<int>(arr_1[k])] = static_cast<W>(k);
        }
