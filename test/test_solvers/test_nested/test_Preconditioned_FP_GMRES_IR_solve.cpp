...
public:
    ...
    void PreconditionedSolveTest(
        ...
        PrecondArgPkg<M, T> precond_args(
            std::make_shared<ILUPreconditioner<M, T>>(A.template cast<T>(), true)
        );
        FP_GMRES_IR_Solve<M, T> precond_gmres_ir(&typed_lin_sys, Tol<T>::roundoff(), args, precond_args);
        precond_gmres_ir.solve();

        if (*show_plots) { precond_gmres_ir.view_relres_plot("log"); }

        EXPECT_TRUE(precond_gmres_ir.check_converged());
        EXPECT_LE(precond_gmres_ir.get_relres(), Tol<T>::krylov_conv_tol());
        EXPECT_LE(precond_gmres_ir.get_iteration(), gmres_ir.get_iteration());

    }

};

TEST_F(Preconditioned_FP_GMRES_IR_Test, DoubleConvergenceTest_ConvDiff64_SOLVER) {
...
TEST_F(Preconditioned_FP_GMRES_IR_Test, SingleConvergenceTest_ConvDiff64_SOLVER) {
...
TEST_F(Preconditioned_FP_GMRES_IR_Test, HalfConvergenceTest_ConvDiff64_SOLVER) {

    fs::path A_path(solve_matrix_dir / fs::path("conv_diff_64_A.csv"));
    fs::path b_path(solve_matrix_dir / fs::path("conv_diff_64_b.csv"));

    PreconditionedSolveTest<MatrixDense, half>(
        A_path, b_path, hlf_GMRES_IR_args, Tol<half>::nested_krylov_conv_tol()
    );
    PreconditionedSolveTest<NoFillMatrixSparse, half>(
        A_path, b_path, hlf_GMRES_IR_args, Tol<half>::nested_krylov_conv_tol()
    );
